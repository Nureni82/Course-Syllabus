# BIT 465: REST API Development Syllabus
  ***BIT 465/Section H1 /5 Credits/Winter 2022***

[Please *review* the **Cascadia College Syllabus Learning Agreement** for more details about Student Rights and Responsibilities ](https://www.cascadia.edu/programs/sla.aspx)

## Course Information
**Course Delievery**: Hybrid (Check the Canvas Home page for details)      
**Learning Materials**: All the learning materials, including exercises and assignments, have been arranged on a lecture basis on Canvas(***cascadia.instructure.com***)   
**Instructor Name**: Nureni Adeyemo   
**How to Contact Me**: Canvas Message is the primary way of reaching me. Alternatively, you can contact me using my Cascadia Email address: ***nadeyemo@cascadia.edu***. *I will respond to your messages or emails not later than 48 hours, except weekends and public holidays.*   
**Office Hours**:  Wednesdays, 3 - 4pm (Remote) – *See the Zoom details on Canvas*

## Course Description
Most modern mobile applications communicate with a server, whether it’s sending game scores, posting updates, downloading videos, or receiving notifications. Students will build a backend system implementing Representational State Transfer (REST) endpoints for creating, reading, updating and deleting data with their apps. The students will learn about Object-Relational Mapping (ORM) and how it can be leveraged to build and interact with database systems.

## Prerequisites
Admission to the BAS-IT program, AND Completion of BIT 340, BIT 372, and BIT 382 with a grade of 2.0 or higher.

## Workplace Context
*How will the outcomes from this class be used in the workplace?*   
*Scenario*:  Bokamoso works on a team that develops mobile applications for iOS and Android.  Their current project requires that their app access data stored in the cloud.  Bokamoso creates a program that runs on a Platform-as-a-Service offering; the program provides a REST API for both iOS and Android mobile apps to use.  Bokamoso utilizes software engineering best practices through the development process, including using version control software such as git, project management techniques and support tools, and both ad hoc and automated testing. Bokamoso successfully delivers the software, enabling her team to successfully complete the mobile app development project.

## Required Textbook
* **Title**: Building RESTful Python Web Services
* **Authors**: Gaston C. Hilter
* **Publisher**: Packt Publishing Limited; 1Ed (October 2016)
* **Language**: English
* **ISBN-13**: 978-1-78646-225-1      

**Additional Material:** *Many other carefully sourced online materials/class notes*

## Student Assessment
Student grades will be determined by the student’s performance on:  
<TABLE>
<TR>
      <TH>Assessment Group</TH>
      <TH>Percentage</TH>
   </TR>
   <TR>
      <TD>Assignments</TD>
      <TD>30%</TD>
   </TR>
   <TR>
      <TD>Exercises</TD>
      <TD>25%</TD>
   </TR>
   <TR>
      <TD>Exam</TD>
      <TD>15%</TD>
   </TR>
   <TR>
      <TD>Final Project</TD>
      <TD>30%</TD>
   </TR>
   <TR>
      <TD><strong>Total</strong></TD>
      <TD><strong>100%</strong></TD>
   </TR>
</TABLE>
<br>
*The number of assessments and the points possible for exams, assignments and other activities are subject to change without notice.*

## Grading Criteria/Standards Scales
<img src="https://github.com/Nureni82/Course-Syllabus/raw/main/assets/images/GradingCriteria.PNG">

## Learning Outcomes
I. **Learn Actively** - *Learning is a personal, interactive process that results in greater expertise and a more comprehensive understanding of the world.*
* Develop and test online backend systems for mobile applications that utilize REST API(s) to communicate with the client apps
* Develop and test database systems that support the REST API backends
* Utilize and apply industry standard security measures throughout the system
* Perform software functionality testing at appropriate stages of the development process
* Debug, diagnose, and fix errors in REST API code
* Demonstrate individual responsibility for learning both inside of and outside of group activities
* Participate in in-class exercises designed to develop an understanding of REST APIs, databases, security, and other topics
* Apply effective conflict resolution strategies in team projects


II.	**Think Critically, Creatively and Reflectively** - *Reason and imagination are fundamental to problem solving and critical examination of ideas.*
* Compare and contrast different options for implementing different (technical) aspects of the overall project
* Reflect on time requirements for the project
* Make trade-off decisions that balance project objectives and project schedule
* Analyze the capability of different software resources (such as frameworks or libraries) for design constraints
* Experiment with new technologies using both innovative and systematic thinking
* Reflect on one's effectiveness in small group work, formulate plans to improve one's performance based on that reflection, and then execute said plan


III. **Communicate with Clarity and Originality** - *The ability to exchange ideas and information is essential to personal growth, productive work, and societal vitality.*
* Learn to listen effectively and respond appropriately to group members during the group process
* Incorporate feedback effectively into ongoing iterations of the project
* Communicate information about the team project in effective, precise and understandable ways
* Execute individual and group presentations to effectively articulate the objectives, design and overall implementation choices


IV. **Interact in Diverse and Complex Environments** - *Successful negotiation through our increasingly complex, interdependent, and global society requires knowledge and awareness of self and others, as well as enhanced interaction skills.*
* Collaborate within team(s) to complete a substantial, course related, project(s)
* Interact within groups to review and assess proposed REST API designs
* Demonstrate competent group process and communication skills
* Demonstrate respect for individual ways of arriving at answers, expressing results and processes
* Use online development and version control tools to coordinate collaborations
* Improve group communication skills through in-class and online activities and team projects
* Engage in leadership roles and responsibilities within the group process


## Objectives
* Personal System setup, including text editor, source code control tools, and testing tools
* Backend systems, using Platform-as-a-Service (PaaS) services such as Amazon Web Service’s Elastic Beanstalk, or Microsoft’s Azure Web Application hosting
* REST API basics, including code to handle HTTP requests and JSON/XML encoding/decoding
* REST API software that utilize databases
* Properly secured REST APIs
* REST APIs testing strategies and techniques, both ad hoc (e.g., using Postman) and automated)
* Version control systems (e.g., git)
* Programming language- and technology-specific topics, as needed
* Work in a team setting to collaborate on all of the abov

## Classroom Conduct
Following these simple guidelines will help me to offer an enjoyable teaching environment for you. You are expected to join class prepared and on time. Joining late and/or unprepared is disrespectful and disruptive both to me and the other students in class. If you must join late, mute your speaker before joining the class. If you have any personal concerns, please feel free to talk to me at the end of class. The official Standards of Student Conduct policy is available in the student handbook.   
Please remember to respect the following list for me and the students in the class.
* Please remember to respect other students and your instructor by not using inappropriate language.
* Please silence your mobile devices such as cell phones.
* Do not play computer games or visit gaming websites during class times.
* No chat or messenger programs during class times.
* Do not use personal audio devices such as MP3 players during class times.
* No Internet surfing, texting, or other mobile device activity that can distract you. 

## Withdrawals
If you decide to drop, it is your responsibility to turn in a withdrawal to Enrollment Services. Do not assume that you will be dropped automatically. Please see the instructional calendar on the college website.

## Late Assignments Policy:   
Each student receives two 48-hour *late work* tokens that can be used only on exercises and assignments, throughout the quarter. After you have used your tokens, **any late exercises and assignments, will automatically lose 5% per day, and up to 25% reduction in their total points.** If you miss a graded activity due to a genuine absence, you must contact me during the first-class period of your return to arrange to complete the work and have it graded. Do not expect me to contact you regarding missed work. Any missed work that remains ungraded at the last day of the semester will receive a zero, regardless of whether the absence was excused.

## Academic Honesty
Cascadia College regards acts of academic dishonesty, including such activities as plagiarism, cheating and/or violations of integrity in information technology, as very serious offenses.  Academic honesty violations include, but are not limited to, copying homework, quiz and/or exam problems, and placing your name on a group project when you did not actively participate. If cheating, plagiarism, or other forms of academic honesty violations are discovered in student’s work, that Lab/lab/assignment will be awarded a zero grade. If the trends continue in more than three graded assignments, the student will be awarded a 0.0 as the final grade for the class.

A formal complaint with detailed description of all such academic dishonesty incidents will be forwarded to the student conduct officer, where a file of such occurrences is maintained. The Student Conduct Officer may institute further action against a student according to the college’s disciplinary policies and procedures. [Click here to see the policies and procedures in the Student Handbook.](https://www.cascadia.edu/academic_resources/handbook.aspx)

## Inclement Weather and Power Outages
If you cannot join the class because of any adverse weather conditions, power outages, or internet issues, you should contact me as soon as possible. Likewise, if I will not be able to join the class for any reason, I will notify you of any cancellation or change as soon as possible. If you cannot join the class due to inclement weather, power outage/internet issues, I will make exception relative to the class attendance requirement, and postpone any scheduled tests, exercises, assignments, and other assessments to a later date.

## COVID-19 Related Protocols and Pivot Plans
Cascadia College is a vaccinated community as mandated by Washington State Government. Although the instruction delivery modality for this class is *mostly* synchronous (Zoom), it is important that the instructor is informed about any sick student (especially if it is COVID), so that exceptions can be made relative to any scheduled assignments. Cascadia College COVID-19 policies and resources are available [here](https://www.cascadia.edu/services/emergency/coronavirus.aspx). Please check the link for regular updates on COVID-19.

---

## Course Schedule for Winter Quarter 2022

**Course Dates:** January 3 – March 18, 2022 

**Class Times:** Wednesdays at 4:45 – 7:50pm 

| Class |          Date          |<p align="left">  Class Activities & Topics </p>                   |
|:-----:|:----------------------:|:---------------------------------------------------------:|
|  1 |  Wed, Jan 5        | <p align="left">**Class Introduction & Introduction to RESTful API**</p>           |
|  2 |  Wed, Jan 12        | <p align="left">**Getting Started with AWS Gateway and HTTP Service**</p>        |
|  3 |  Wed, Jan 19       | <p align="left">**DynamoDB and Lambda Function** </p>                     |
|  4 |  Wed, Jan 26       |<p align="left"> **API Gateway to Lambda** </p>                          |
|  5 |  Wed, Feb 2       | <p align="left">**Client SDK from Swagger**</p>                             |
|  6 |  Wed, Feb 9       | <p align="left">**Developing RESTful API with Django**</p>                          |
|  7 |  Wed, Feb 16     | <p align="left">**Working with Class-Based Views and Hyperlinked API**</p>                      |
|  8 |  Wed, Feb 23          | <p align="left">**Authenticating an API with Django**</p>                   |
|  9 |  Wed, Mar 2          | <p align="left">**API Throttling & Development with Django**</p>                |
| 10 |  Wed, Mar 9           | <p align="left">**Exam**</p>                   |
| 11 |  Wed, Mar 16          |<p align="left">**Final project**</p> |                                          |
---
*The due dates for the exercises, assignments, exams, and projects are available on the Learning Management System(Canvas)*
